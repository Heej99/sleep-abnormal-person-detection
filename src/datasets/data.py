...
class OurData(BaseData):
    def __init__(self, root_dir, pattern=None, n_proc=1, limit_size=None, config=None):
    ...
    def load_all(self, root_dir, pattern=None):
        ...
        all_df = pd.read_csv(f'{root_dir}/our_data_train_with_state.csv', index_col=0)
        if pattern == 'TRAIN':
           labels_df = None
        elif pattern == 'TEST':
            labels_df = None
